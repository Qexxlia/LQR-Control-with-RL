...
class StateCallback(BaseCallback):
    ...
    def _on_rollout_end(self):
        ...
        # ax.set_ylim(min(self.map_limits[0,0:6]), max(self.map_limits[1,0:6]))
        fig.legend()
        ...
        # ax1.set_ylim(min(self.map_limits[0,6:9]), max(self.map_limits[1,6:9]))
        fig1.legend()
        ...
        # ax2.set_ylim(self.t_step_limits[0], self.t_step_limits[1])
        ...
        time = t[-1]
        self.logger.record("time/ep_time_elapsed", time, exclude=("stdout", "log", "json", "csv"))

        deltaV_punishment= info[-1].get('deltaV_punishment')
        velocity_punishment = info[-1].get('velocity_punishment')
        distance_punishment = info[-1].get('distance_punishment')
        time_punishment = info[-1].get('time_punishment')
        truncated_punishment = info[-1].get('truncated_punishment')

        self.logger.record("reward/ep_delta_v_punishment", deltaV_punishment, exclude=("stdout", "log", "json", "csv"))
        self.logger.record("reward/ep_time_punishment", time_punishment, exclude=("stdout", "log", "json", "csv"))
        self.logger.record("reward/ep_distance_punishment", distance_punishment, exclude=("stdout", "log", "json", "csv"))
        self.logger.record("reward/ep_velocity_punishment", velocity_punishment, exclude=("stdout", "log", "json", "csv"))
        self.logger.record("reward/ep_trucated_punishment", truncated_punishment, exclude=("stdout", "log", "json", "csv"))
