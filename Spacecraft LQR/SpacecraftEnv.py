...
class SpacecraftEnv(gym.Env):
    ...
    def step(self, action):
        ...
        sol = scd.simulate(self.state, time_range, q_weights, r_weights, self.A, self.u_max)
        ...
        #$$ Reward Calculation
        if converged or no_deltaV or truncated or terminated:
            reward = 0
            time_punishment = (self.current_time - self.initial_time)/2
            ...
            for i in range(self.pos.shape[1]):
                distance_punishment += np.linalg.norm(self.pos[:,i])

            if no_deltaV:
                deltaV_punishment = 100
            if truncated:
                truncated_punishment = 100
    ...
    def range_state(self):
        ...
        theta = np.random.uniform(theta_initial - self.variance, theta_initial + self.variance)
        phi = np.random.uniform(phi_initial - self.variance, phi_initial + self.variance)
